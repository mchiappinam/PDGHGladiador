...
public class Comando implements CommandExecutor {
		...
		if(cmd.getName().equalsIgnoreCase("gladiador")) {
			if(args.length==0) {
				...
				if(args[0].equalsIgnoreCase("setarenamenor")) {
					...
					plugin.saveConfig();
					sender.sendMessage("§3§l[Gladiador] §oArena menor marcada!");
					return true;
				}
	...
	private void sendHelp(Player p) {
		...
		p.sendMessage("§2/gladidor setfim -§a- Marca local do fim do evento gladiador");
		p.sendMessage("§2/gladidor info -§a- Mostra quantos jogadores estão dentro do evento gladiador");
