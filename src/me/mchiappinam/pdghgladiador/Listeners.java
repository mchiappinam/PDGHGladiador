...
public class Listeners implements Listener {
	...
	public void onPlayerRespawn(PlayerRespawnEvent e) {
	    if(e.getPlayer().getWorld()==plugin.getServer().getWorld("world_eventos")) {
        	if (plugin.saida.getWorld()!=null) {
	        	e.getPlayer().sendMessage("§3§l[Gladiador] §cVocê morreu :(");
	...
	private void onDeath(PlayerDeathEvent e) {
		if(e.getEntity().getKiller() instanceof Player) {
			...
			if((plugin.getGladiadorEtapa()!=3)||(plugin.pvpOffNovaArena))
	...
	private void onPCmd(PlayerCommandPreprocessEvent e) {
		if(plugin.getGladiadorEtapa()!=0) {
			if(e.getMessage().toLowerCase().startsWith("/clan")) {
				if(!e.getMessage().toLowerCase().startsWith("/clan create")&&!e.getMessage().toLowerCase().startsWith("/clan invite")&&!e.getMessage().toLowerCase().startsWith("/clan roster")) {
					e.getPlayer().sendMessage("§c§lComando bloqueado no evento gladiador!");
					e.getPlayer().sendMessage("§2§lOs únicos comandos liberados são: §b§l/clan create§2§l, §b§l/clan invite §2§le §b§l/clan roster");
					e.setCancelled(true);
				}
			}else if(e.getMessage().toLowerCase().startsWith("/ret")) {
				e.getPlayer().sendMessage("§c§lComando bloqueado no evento gladiador!");
				e.setCancelled(true);
